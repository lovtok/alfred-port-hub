        ...
        categoryLoad_portfolio: function() {
            ...
            $('.rainbow-portfolio-filter .messonry-button').on('click', '.messonry-button .rn-btn', function(e) {
                ...
                if (cat_slug !== post_id_parent_class.attr('data-current-cat')) {
                    paged = 1;
                    prev_post_ids = [];
                    post_id_parent_class.attr('data-current-cat', cat_slug);
                    contentWrap.empty()
                }
                $.ajax({
                    url: RainbowObj.ajaxurl,
                    ...
                    success: function(resp) {
                        if (resp.html.length > 0) {
                            ...
                            if (resp.posts_per_page > resp.post_ids_json.length) {
                                ...
                                loadmore.find('a').text('No more projects').addClass('d-none').attr('disabled', !0)
                            } else {
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        if (resp.html.length > 0) {
                            ...
                            if (resp.remaining == !1) {
                                loadmore.addClass('d-none');
                                loadmore.find('a').text('No data remaining').addClass('d-none').attr('disabled', !0)
                            } else {
                                loadmore.removeClass('d-none');
